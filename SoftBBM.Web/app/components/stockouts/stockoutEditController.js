(function (app) {
    app.controller('stockoutEditController', stockoutEditController);

    stockoutEditController.$inject = ['apiService', '$window',  '$scope', 'notificationService', '$state', '$stateParams'];

    function stockoutEditController(apiService, $window, $scope, notificationService, $state, $stateParams) {
        $scope.stockin = {
        ...
        function saveBook() {
            if ($scope.bookDetails.length > 0) {
                ...
                $scope.stockin.Total = sumQuantity();
        ...
        function loadBookDetail() {
            ...
            apiService.get('/api/stockin/detaileditstockout', config, function (result) {
                if (result.data.FromBranchStatusId == '06' || result.data.FromBranchStatusId == '02' || result.data.CategoryId!='03') {
                ...
                $window.document.title = "Cập nhật đơn xuất kho - " + $scope.stockin.Id;
                ...
                $.each($scope.branches, function (index, value) {
                    if (value.Id == result.data.ToBranchId) {
